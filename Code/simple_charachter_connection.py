...
def create_weighted_graph(pair_sentences, dict_names_id, threshold_count=3):
    G = nx.Graph()
    for pair, lst_sentences_indexes in pair_sentences.items():
        count = lst_sentences_indexes
        # count = len(lst_sentences_indexes)
...
def main():
    ...
    #
    # # plot_simple_connections(pair_sentences, dict_names_id, threshold_count=10)
    # G, pos = plot_page_rank(pair_sentences, dict_names_id, threshold_count=15)
    # plot_louvain_communities(G, pos, resolution=1.7)
    # plot_leiden_communities_with_weights(G, pos, resolution=1.7)

    # todo: for each sentence in the set_sentences do a sentiment analysis. create dict to match index of sentence to
    # its sentiment.
